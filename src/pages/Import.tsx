...
export default function Import() {
  ...
  async function save() {
    ...
    try {
      ...
      if (useNewPeriod || !pid) {
        pid = generateId()
        const label = newPeriodLabel.trim() || `${new Date(newPeriodYear, newPeriodMonth - 1).toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}`
      ...
      // Also save hidden (reversal pairs) with hidden=true
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="bg-blue-700 text-white px-4 py-4">
          <h1 className="text-lg font-bold">Review Parsed Transactions</h1>
          <p className="text-sm text-blue-200 mt-0.5">
            {visibleParsed.filter((t) => !(t as any)._hidden).length} transactions found
        ...
        <div className="mx-4 mt-4 bg-white rounded-xl shadow-sm p-4">
          <h2 className="font-semibold text-gray-900 mb-3">Assign to Period</h2>
        ...
        <div className="fixed bottom-16 left-0 right-0 px-4 pb-2">
          <div className="bg-white rounded-xl shadow-lg p-3 flex items-center gap-3">
            ...
            <button
              onClick={save}
              disabled={!useNewPeriod && !periodId}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-blue-700 text-white px-4 py-4">
        ...
        <p className="text-sm text-blue-200 mt-0.5">Upload PDF (billed) or screenshot (unbilled)</p>
      </div>
